...
async def refresh_quote_from_qbo(quote_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    try:
        estimate = await fetch_estimate_by_doc_number_or_id(db, quote.qbo_estimate_id)
        quote = upsert_quote_from_qbo_estimate(db, estimate)
        db.commit()
    except QboError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
