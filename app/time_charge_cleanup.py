...
async def _fetch_time_activities(db: Session, scan: ScanRequest) -> tuple[list[dict[str, Any]], bool]:
    ...
    # Continue until QuickBooks returns an empty page. Advancing by the actual
    # number returned is safer than assuming every non-final page contains the
    # full requested page size.
    while True:
...
async def _fetch_latest_time_activities(db: Session, ids: list[str]) -> dict[str, dict[str, Any]]:
    """Retrieve the current QBO version of a small set of TimeActivity records."""
    cleaned_ids = _dedupe_ids(ids)
    if not cleaned_ids:
        return {}
    if len(cleaned_ids) > BATCH_SIZE:
        raise ValueError(f"At most {BATCH_SIZE} TimeActivity IDs can be refreshed at once.")

    def escape_query_literal(value: str) -> str:
        return value.replace("\\", "\\\\").replace("'", "\\'")

    quoted_ids = ", ".join(f"'{escape_query_literal(record_id)}'" for record_id in cleaned_ids)
    query = f"select * from TimeActivity where Id in ({quoted_ids}) maxresults {len(cleaned_ids)}"
...
async def delete_time_charge_chunk(payload: DeleteChunkRequest, request: Request, db: Session = Depends(get_db)):
    ...
    scan_safe_records: list[dict[str, Any]] = []
    for record_id in ids:
        ...
        if record.get("billable_status") == PROTECTED_BILLABLE_STATUS:
            ...
            scan_safe_records.append(record)
    db.commit()

    for offset in range(0, len(scan_safe_records), BATCH_SIZE):
        scanned_batch = scan_safe_records[offset : offset + BATCH_SIZE]
        batch_ids = [str(record["id"]) for record in scanned_batch]

        # Intuit recommends retrieving the latest entity before a delete so the
        # current SyncToken is used. We also refuse to delete anything that was
        # changed after the user's scan and CSV backup.
        try:
            latest_by_id = await _fetch_latest_time_activities(db, batch_ids)
        except (QboError, ValueError) as exc:
            message = f"Could not refresh the latest QuickBooks records before deletion: {exc}"
            for record in scanned_batch:
                _add_log(
                    db,
                    scan_id=scan.id,
                    batch_request_id=None,
        ...
        for scanned_record in scanned_batch:
            record_id = str(scanned_record["id"])
            latest_record = latest_by_id.get(record_id)
            if latest_record is None:
                message = "The TimeActivity no longer exists in QuickBooks."
                _add_log(
                    db,
                    scan_id=scan.id,
                    batch_request_id=None,
                ...
                results.append({"id": record_id, "status": "already_missing", "message": message})
                continue

            if latest_record.get("billable_status") == PROTECTED_BILLABLE_STATUS:
                message = "Protected because the TimeActivity is now marked HasBeenBilled in QuickBooks."
                _add_log(
                    db,
                    scan_id=scan.id,
                    batch_request_id=None,
            ...
            if str(latest_record.get("sync_token") or "") != str(scanned_record.get("sync_token") or ""):
                message = "This TimeActivity changed after the scan. Run a new scan and download a new backup before deleting it."
                _add_log(
                    db,
                    scan_id=scan.id,
                    batch_request_id=None,
                ...
                results.append({"id": record_id, "status": "changed_since_scan", "message": message})
                continue

            delete_records.append(latest_record)
        db.commit()

        if not delete_records:
            continue

        delete_ids = [str(record["id"]) for record in delete_records]
        request_id = _deterministic_request_id(scan.id, delete_ids)
        request_items = [
            {
                "bId": f"d{index}",
                "operation": "delete",
    ...
    counts = {
        ...
        "changed_since_scan": sum(1 for result in results if result["status"] == "changed_since_scan"),
        "failed": sum(1 for result in results if result["status"] in {"failed", "unknown"}),
    }
    response = JSONResponse({"results": results, "counts": counts, "recommended_delay_ms": 10_000})
    return _no_store(response)
